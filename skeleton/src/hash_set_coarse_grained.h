...
private:
  ...
  // Mutex for course grained locking
  std::mutex mutex_;
  std::atomic<size_t> setSize_;
  ...
  bool Policy() { return setSize_.load() / capacity_ > 4; }
...
public:
  explicit HashSetCoarseGrained(size_t capacity) : capacity_(capacity) {
